...
class ObjectDetector(private val service: AccessibilityService) {
    ...
    fun detectObjects(scriptId: String, forceRefresh: Boolean = false): List<DetectedObject> {
        ...
        Logger.info(
            "Detector: ${sorted.size} unique object(s) for script=$scriptId " +
            ...
            if (visible && !visibleBounds.isEmpty &&
                ...
                keywords.forEach { (kw, exact) ->
                    ...
                    if (matches) {
                        ...
                        // FIX: wrap bonus terms in parentheses — Kotlin operator precedence
                        // bug: `0f + if(...) X else 0f` was parsed as `(0f + if) ... else 0f`
                        val conf = (
                            base +
